...
from audit.models import RequestLog, AuditLog, AuditConfiguration, AuditEventType
...
class RequestPathCategoryFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if value == 'other':
            return (
                queryset
                .exclude(request_path__startswith='/health')
                .exclude(request_path__startswith='/api')
                .exclude(request_path__startswith='/cia')
                .exclude(request_path__startswith='/favicon')
            )
        return queryset
...
class RequestLogAdmin(admin.ModelAdmin):
    ...
    def related_audits_link(self, obj):
        ...
        return '-'
    related_audits_link.short_description = _('Audit Logs')
...
class AuditLogAdmin(admin.ModelAdmin):
    ...
    def summary(self, obj):
        model_label = obj.content_type.model if obj.content_type else _('Unknown Model')
        if obj.event_type == AuditEventType.CREATE:
            action = _('Created')
        elif obj.event_type == AuditEventType.UPDATE:
            action = _('Updated')
        elif obj.event_type == AuditEventType.DELETE:
            action = _('Deleted')
        else:
    ...
    def view_request_link(self, obj):
        ...
        try:
            ...
            return format_html("<a href='{}' target='_blank'>{}</a>", url, _('View Request'))
    ...
    def view_object_link(self, obj):
        ...
        try:
            ...
            if instance:
                ...
                return format_html("<a href='{}' target='_blank'>{}</a>", url, _('View Object'))
            return format_html("<span style='color: #999;'>{}</span>", _('Deleted'))
        except Exception:
            return '-'
